...
import com.skyd.imomoe.util.download.datasource.DataSourceDownloadHelper.Companion.downloadHashMap
import com.skyd.imomoe.util.download.datasource.DataSourceDownloadNotificationReceiver.Companion.DOWNLOAD_DATA_SOURCE_NOTIFICATION_ID
...
class DataSourceDownloadService : Service() {
    private val downloadServiceHashMap: HashMap<String, DataSourceDownloadServiceDataBean> = HashMap()
    ...
    override fun onStartCommand(intent: Intent, flags: Int, startId: Int): Int {
        ...
        downloadServiceHashMap[key] = DataSourceDownloadServiceDataBean(url, totalNotificationId++)
    ...
    private fun createNotification(key: String) {
        ...
        val stopIntent = Intent(this, DataSourceDownloadNotificationReceiver::class.java)
        stopIntent.action = "notification_canceled"
        stopIntent.putExtra(DOWNLOAD_DATA_SOURCE_NOTIFICATION_ID, notificationId)
        ...
        clickIntent.putExtra(DOWNLOAD_DATA_SOURCE_NOTIFICATION_ID, notificationId)
    ...
    private fun downloadAnime(
        ...
        FileDownloader.getImpl().create(param)
            .setPath(DataSourceManager.getJarDirectory(), true)
    ...
    inner class DataSourceDownloadServiceDataBean(
        var url: String,
